...
from kslot_knapsack_input_GEN import Ui_MainWindow
import knapsack
...
class win(QMainWindow):
    ...
    def setupUI(self):
        ...
        self.ui.slot_box.addItems(str(i) for i in range(1, MAX_SLOTS+1))
        self.ui.slot_box.setCurrentIndex(DEFAULT_SLOTS-1)
    ...
    def on_done_btn(self):
        slots, size = int(self.ui.slot_box.currentText()), int(self.ui.size_box.currentText())
        weights = []
        for item_row in range(self.ui.input_table.rowCount()):
            weights.append(int(self.ui.input_table.item(item_row, 1).text()))
        self.main_window.alg = knapsack.KSlotKnapSack(weights, size, slots)
